...
namespace ERManagementSystem.Services
{
    public class ExaminationService : IExaminationService
    {
        ...
        public int RequestDoctor(int visitID)
            => RequestDoctorAsync(visitID).GetAwaiter().GetResult();

        public async Task<int> RequestDoctorAsync(int visitID)
        {
            var triage = await triageRepository.GetByVisitIdAsync(visitID);
            ...
            var triageParameters = await triageParamsRepo.GetByTriageIdAsync(triage.Triage_ID);
            ...
            await stateManagementService.ChangeVisitStatusAsync(visitID, ER_Visit.VisitStatus.WAITING_FOR_DOCTOR);
        ...
        public void SaveExamination(Examination exam)
            => SaveExaminationAsync(exam).GetAwaiter().GetResult();

        public async Task SaveExaminationAsync(Examination exam)
        {
            await examRepository.AddAsync(exam);
            await stateManagementService.ChangeVisitStatusAsync(exam.Visit_ID, ER_Visit.VisitStatus.IN_EXAMINATION);
